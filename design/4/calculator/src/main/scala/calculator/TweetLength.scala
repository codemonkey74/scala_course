...
object TweetLength {
  ...
  def tweetRemainingCharsCount(tweetText: Signal[String]): Signal[Int] = {
    Signal.apply(MaxTweetLength - tweetLength(tweetText.apply()))
  }

  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] = Signal(remainingCharsCount.apply() match {
    case c if c >=15 => "green"
    case c if c >=0 && c <= 14 => "orange"
    case _ => "red"
  })
