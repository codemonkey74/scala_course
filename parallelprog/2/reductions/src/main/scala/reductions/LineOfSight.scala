...
object LineOfSight {
  ...
  object Node {
    def fromTuple(t: (Tree, Tree)) = Node(t._1, t._2)
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    var maxA = input(from) / Math.max(1, from)
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int): Unit = {
    Math.max(1, from).until(until).foreach { i =>
      output(i) = max(output(i-1), max(input(i)/i, startingAngle))
    }
  }
